    ...
    while (1)
    {
        for (int i = 0; i < WINDOW_SIZE; ++i)
        {
            ...
            if ( len <= 0)
            {
                ...
                sendto(sockfd, Sending[i], TCP_HDR_SIZE,  0,
                        (const struct sockaddr *)&serveraddr, serverlen);
                break;
            }
            send_base = next_seqno;
            next_seqno = send_base + len;
            Sending[i] = make_packet(len);
            memcpy(Sending[i]->data, buffer, len);
            Sending[i]->hdr.seqno = send_base;
            ...
            do {
                 ...
                 */
                if(sendto(sockfd, Sending[i], TCP_HDR_SIZE + get_data_size(Sending[i]), 0,
                            ( const struct sockaddr *)&serveraddr, serverlen) < 0)
                {
                    error("sendto");
                }
